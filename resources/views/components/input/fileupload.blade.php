@props([
    'file' => null,
    'accept' => 'image/jpg,image/jpeg,image/png,application/pdf,application/xls,application/xlsx',
    'multiple' => false,
    'mode' => 'attachment',
    'show' => 'w-20 h-20 rounded-full',
])
...
<div x-data="{
    ...
    handleFiles() {
        if (this.isMultiple === true) {
            @this.uploadMultiple('{{ $attributes->wire('model')->value }}', this.$refs.input.files, () => {}, () => {}, (event) => {
                this.progress = event.detail.progress || 0
            })
        } else {
            @this.upload('{{ $attributes->wire('model')->value }}', this.$refs.input.files[0], () => {}, () => {}, (event) => {
                this.progress = event.detail.progress || 0
            });
        }
    }
    ...
    @if (!$file || $mode === 'preview')
        ...
        <label for="file-{{ $randomId }}"
            ...
            {{ $mode === 'preview' ? 'border group ' . $show : 'border-2 w-full pl-3 pr-4 py-2 border-dashed' }}"
            wire:loading.class="pointer-events-none" :class="{ 'border-indigo-300': isFocused === true }">
            {{-- hack to get the progress of upload file --}}
            <input type="hidden" name="{{ $attributes->wire('model')->value }}" {{ $attributes->wire('model') }} />

            <input type="file" id="file-{{ $randomId }}"
                class="absolute inset-0 cursor-pointer opacity-0 text-transparent sr-only" accept="{{ $accept }}"
                :multiple="isMultiple" x-ref="input" x-on:change.prevent="handleFiles" x-on:focus="isFocused = true"
            ...
            <div wire:loading.flex wire:target="{{ $attributes->wire('model')->value }}" wire:loading.class="w-full">
                @if ($mode === 'preview' && $file)
                    <div
                        class="select-none text-sm text-indigo-500 flex flex-1 items-center justify-center text-center p-4">
                    ...
                    <div class="relative w-full overflow-hidden shadow-inner" style="padding-bottom: 100%">
                        @if (collect(['jpg', 'png', 'jpeg', 'webp'])->contains($file->getClientOriginalExtension()))
                            ...
                            <div
                                ...
                                <svg class="h-6 w-6 text-gray-100" xmlns="http://www.w3.org/2000/svg" fill="none"
            ...
            <div class="flex items-center justify-center flex-1 px-4 py-2" wire:loading.class="hidden"
                wire:target="{{ $attributes->wire('model')->value }}">
                @if ($slot->isEmpty())
                    @if ($multiple)
                        <svg class="-mr-5 -mt-2 transform -rotate-6 h-8 w-8 text-gray-300 group-hover:text-indigo-300 transition duration-500 ease-in-out"
                        ...
                        <svg class="transition duration-500 ease-in-out relative h-8 w-8 transform rotate-3 text-gray-400 group-hover:text-indigo-500"
                            xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 256 256">
                            <rect width="256" height="256" fill="none"></rect>
                            <path
                    ...
                    @else
                        @if ($mode === 'preview')
                            <svg class="h-8 w-8 text-gray-300 group-hover:text-indigo-500"
                ...
                @foreach ($file as $key => $f)
                    <div class="py-3 flex {{ !$loop->last ? 'border-b border-gray-200' : '' }}">
                        <div class="w-16 mr-4 flex-shrink-0 shadow-xs rounded-lg">
                            ...
                            @else
                                <div
                                    class="w-16 h-16 bg-gray-100 text-blue-500 flex items-center justify-center rounded-lg border border-gray-100">
                        ...
                        <div>
                            ...
                            <div class="text-sm font-medium truncate w-40 md:w-auto">{{ $f->getClientOriginalName() }}
                            </div>
                            <div class="flex items-center space-x-1">
                                <div class="text-gray-400 text-xs">&bull;</div>
                                <div class="text-xs text-gray-500 uppercase">{{ $f->getClientOriginalExtension() }}
                                </div>
                            </div>

                            <button wire:key="remove-attachment-{{ $f->getFilename() }}" wire:loading.attr="disabled"
                                type="button"
                    ...
                    <div class="mt-3 flex">
                        ...
                        <div>
                            @error($attributes->wire('model')->value)
                                <p class="text-sm text-red-600" class="mb-2">{{ $message }}</p>
                            @enderror
                            <div class="text-sm font-medium truncate w-40 md:w-auto">
                                {{ $file->getClientOriginalName() }}</div>
                            <div class="flex items-center space-x-1">
                                <div class="text-gray-400 text-xs">&bull;</div>
                                <div class="text-xs text-gray-500 uppercase">{{ $file->getClientOriginalExtension() }}
                                </div>
                            </div>
